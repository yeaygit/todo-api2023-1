...
import com.example.todo.userapi.exception.NoRegisteredArgumentException;
...
public class UserApiController {
    ...
    public ResponseEntity<?> checkEmail(String email){
        if(email==null||email.trim().equals("")){
            return ResponseEntity.badRequest().body("이메일을 전달해주세요");
        }//required true이기 때문에 지금은 작동하지 않음 하지만 false를 대비해 넣는 코드
    ...
    public ResponseEntity<?> signIn(@Validated @RequestBody LoginRequestDTO requestDTO){
        ...
        try {
            LoginResponseDTO userInfo = userService.getByCredentials(requestDTO.getEmail(), requestDTO.getPassword());
            return ResponseEntity
                    .ok()
                    .body(userInfo);
            ...
            return ResponseEntity
                    .badRequest()
                    .body(LoginResponseDTO.builder().message(e.getMessage()).build());
        }
    }
